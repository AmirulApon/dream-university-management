...
// Show messages
if ( $message === 'added' ) {
	echo '<div class="notice notice-success"><p>' . esc_html__( 'Teacher added successfully!', 'dream-university-management' ) . '</p></div>';
} elseif ( $message === 'updated' ) {
	echo '<div class="notice notice-success"><p>' . esc_html__( 'Teacher updated successfully!', 'dream-university-management' ) . '</p></div>';
} elseif ( $message === 'deleted' ) {
	echo '<div class="notice notice-success"><p>' . esc_html__( 'Teacher deleted successfully!', 'dream-university-management' ) . '</p></div>';
...
if ( $action === 'add' || $action === 'edit' ) {
	$teacher = null;
	if ( $action === 'edit' && $id > 0 ) {
		$teacher = DUM_Teacher::get( $id );
		if ( ! $teacher ) {
			echo '<div class="notice notice-error"><p>' . esc_html__( 'Teacher not found.', 'dream-university-management' ) . '</p></div>';
			return;
		}
	}
	?>
	<div class="wrap">
		<h1><?php echo $action === 'add' ? esc_html__( 'Add Teacher', 'dream-university-management' ) : esc_html__( 'Edit Teacher', 'dream-university-management' ); ?></h1>
		
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			<?php wp_nonce_field( $action === 'add' ? 'dum_add_teacher' : 'dum_edit_teacher' ); ?>
			<input type="hidden" name="action" value="<?php echo $action === 'add' ? 'dum_add_teacher' : 'dum_edit_teacher'; ?>">
			<?php if ( $action === 'edit' ) : ?>
			...
			<table class="form-table">
				<tr>
					<th><label for="teacher_id"><?php esc_html_e( 'Teacher ID', 'dream-university-management' ); ?> <span class="required">*</span></label></th>
					<td><input type="text" id="teacher_id" name="teacher_id" value="<?php echo $teacher ? esc_attr( $teacher->teacher_id ) : ''; ?>" class="regular-text" required></td>
				</tr>
				<tr>
					<th><label for="first_name"><?php esc_html_e( 'First Name', 'dream-university-management' ); ?> <span class="required">*</span></label></th>
					<td><input type="text" id="first_name" name="first_name" value="<?php echo $teacher ? esc_attr( $teacher->first_name ) : ''; ?>" class="regular-text" required></td>
				</tr>
				<tr>
					<th><label for="last_name"><?php esc_html_e( 'Last Name', 'dream-university-management' ); ?> <span class="required">*</span></label></th>
					<td><input type="text" id="last_name" name="last_name" value="<?php echo $teacher ? esc_attr( $teacher->last_name ) : ''; ?>" class="regular-text" required></td>
				</tr>
				<tr>
					<th><label for="email"><?php esc_html_e( 'Email', 'dream-university-management' ); ?> <span class="required">*</span></label></th>
					<td><input type="email" id="email" name="email" value="<?php echo $teacher ? esc_attr( $teacher->email ) : ''; ?>" class="regular-text" required></td>
				</tr>
				<tr>
					<th><label for="phone"><?php esc_html_e( 'Phone', 'dream-university-management' ); ?></label></th>
					<td><input type="text" id="phone" name="phone" value="<?php echo $teacher ? esc_attr( $teacher->phone ) : ''; ?>" class="regular-text"></td>
				</tr>
				<tr>
					...
					<td>
						...
						<div class="dum-image-upload-wrapper">
							...
							<div class="dum-image-preview" style="margin-bottom: 10px;">
								<?php if ( $image_url ) : ?>
									<img src="<?php echo esc_url( $image_url ); ?>" alt="<?php esc_attr_e( 'Teacher Photo', 'dream-university-management' ); ?>" style="max-width: 150px; height: auto; border: 1px solid #ddd; padding: 5px; display: block; margin-bottom: 10px;" id="image_preview">
								<?php else : ?>
				...
				<tr>
					...
					<td>
						...
						$selected_faculty_id = $teacher ? intval( $teacher->faculty_id ) : 0;
						?>
					...
					<td>
						<select id="department_id" name="department_id">
							...
							if ( $selected_faculty_id > 0 ) {
								...
								$selected_department_id = $teacher ? intval( $teacher->department_id ) : 0;
						...
						<input type="text" id="department" name="department" value="<?php echo $teacher ? esc_attr( $teacher->department ) : ''; ?>" class="regular-text" placeholder="<?php esc_attr_e( 'Optional: Old department field', 'dream-university-management' ); ?>">
					</td>
				</tr>
				<tr>
					<th><label for="designation"><?php esc_html_e( 'Designation', 'dream-university-management' ); ?></label></th>
					<td><input type="text" id="designation" name="designation" value="<?php echo $teacher ? esc_attr( $teacher->designation ) : ''; ?>" class="regular-text"></td>
				</tr>
				<tr>
					<th><label for="hire_date"><?php esc_html_e( 'Hire Date', 'dream-university-management' ); ?></label></th>
					<td><input type="date" id="hire_date" name="hire_date" value="<?php echo $teacher ? esc_attr( $teacher->hire_date ) : ''; ?>" class="regular-text"></td>
				</tr>
				<tr>
					<th><label for="status"><?php esc_html_e( 'Status', 'dream-university-management' ); ?></label></th>
					<td>
						<select id="status" name="status">
							<option value="active" <?php echo $teacher && $teacher->status === 'active' ? 'selected' : ''; ?>><?php esc_html_e( 'Active', 'dream-university-management' ); ?></option>
							<option value="inactive" <?php echo $teacher && $teacher->status === 'inactive' ? 'selected' : ''; ?>><?php esc_html_e( 'Inactive', 'dream-university-management' ); ?></option>
						</select>
					</td>
				</tr>
			</table>
			
			<?php submit_button( $action === 'add' ? __( 'Add Teacher', 'dream-university-management' ) : __( 'Update Teacher', 'dream-university-management' ) ); ?>
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=dum-teachers' ) ); ?>" class="button"><?php esc_html_e( 'Cancel', 'dream-university-management' ); ?></a>
		</form>
	...
	$teachers = DUM_Teacher::get_all( array( 
		'search' => $search,
	...
	<div class="wrap">
		...
		<a href="<?php echo esc_url( admin_url( 'admin.php?page=dum-teachers&action=add' ) ); ?>" class="page-title-action"><?php esc_html_e( 'Add New', 'dream-university-management' ); ?></a>
		
		<hr class="wp-header-end">
		
		<div class="dum-search-box">
			<form method="get" action="">
				<input type="hidden" name="page" value="dum-teachers">
				<p class="search-box">
					...
					<?php if ( ! empty( $search ) || $status_filter !== 'all' ) : ?>
						<a href="<?php echo esc_url( admin_url( 'admin.php?page=dum-teachers' ) ); ?>" class="button"><?php esc_html_e( 'Clear', 'dream-university-management' ); ?></a>
					<?php endif; ?>
				</p>
			</form>
		</div>
		...
		<table class="wp-list-table widefat fixed striped">
			<thead>
				<tr>
					...
					<th><?php esc_html_e( 'Teacher ID', 'dream-university-management' ); ?></th>
					...
					<th><?php esc_html_e( 'Designation', 'dream-university-management' ); ?></th>
			...
			<tbody>
				<?php if ( empty( $teachers ) ) : ?>
					<tr>
						<td colspan="10"><?php esc_html_e( 'No teachers found.', 'dream-university-management' ); ?></td>
					</tr>
				<?php else : ?>
					<?php foreach ( $teachers as $teacher ) : ?>
						...
						$faculty_name = '';
						$department_name = '';
						if ( ! empty( $teacher->faculty_id ) ) {
							$faculty = DUM_Faculty::get( $teacher->faculty_id );
							$faculty_name = $faculty ? $faculty->faculty_name : '';
						}
						if ( ! empty( $teacher->department_id ) ) {
							$department = DUM_Department::get( $teacher->department_id );
							$department_name = $department ? $department->department_name : '';
						}
						// Fallback to old department field if new one is empty
						if ( empty( $department_name ) && ! empty( $teacher->department ) ) {
							$department_name = $teacher->department;
						}
						$image_url = ! empty( $teacher->image ) ? esc_url( $teacher->image ) : '';
						?>
						<tr>
							<td><strong><?php echo esc_html( $teacher->id ); ?></strong></td>
							<td>
								<?php if ( $image_url ) : ?>
									<img src="<?php echo esc_url( $image_url ); ?>" alt="<?php echo esc_attr( $teacher->first_name . ' ' . $teacher->last_name ); ?>" style="width: 50px; height: 50px; object-fit: cover; border-radius: 4px;">
								<?php else : ?>
									<span class="dashicons dashicons-businessman" style="font-size: 40px; color: #ccc;"></span>
								<?php endif; ?>
							</td>
							...
							<td><?php echo esc_html( $teacher->designation ); ?></td>
							<td>
								<span class="dum-status <?php echo esc_attr( $teacher->status ); ?>">
									<?php echo esc_html( ucfirst( $teacher->status ) ); ?>
								</span>
							</td>
							<td>
								<a href="<?php echo esc_url( admin_url( 'admin.php?page=dum-teachers&action=edit&id=' . $teacher->id ) ); ?>"><?php esc_html_e( 'Edit', 'dream-university-management' ); ?></a> |
								<a href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=dum_delete_teacher&id=' . $teacher->id ), 'dum_delete_teacher' ) ); ?>" onclick="return confirm('<?php esc_attr_e( 'Are you sure you want to delete this teacher?', 'dream-university-management' ); ?>');"><?php esc_html_e( 'Delete', 'dream-university-management' ); ?></a>
							</td>
						</tr>
					<?php endforeach; ?>
				<?php endif; ?>
