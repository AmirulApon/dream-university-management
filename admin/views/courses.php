...
if ( $is_action || $has_filter_params ) {
	if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'dreaunma-course-view' ) ) {
		wp_die( esc_html__( 'Security check failed. Please refresh the page and try again.', 'dream-university-management' ) . ' <a href="' . esc_url( admin_url( 'admin.php?page=dreaunma-courses' ) ) . '">' . esc_html__( 'Go Back', 'dream-university-management' ) . '</a>' );
	}
}
...
if ( $action === 'add' || $action === 'edit' ) {
	...
	<div class="wrap">
		...
		<a href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin.php?page=dreaunma-courses&action=add' ), 'dreaunma-course-view' ) ); ?>" class="page-title-action"><?php esc_html_e( 'Add New', 'dream-university-management' ); ?></a>
		...
		<div class="dreaunma-search-box">
			<form method="get" action="">
				...
				<input type="hidden" name="_wpnonce" value="<?php echo esc_attr( wp_create_nonce( 'dreaunma-course-view' ) ); ?>">
				<p class="search-box">
				...
				<?php else : ?>
					<?php foreach ( $courses as $course ) : ?>
						...
						<tr>
							...
							<td>
								<a href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin.php?page=dreaunma-courses&action=edit&id=' . $course->id ), 'dreaunma-course-view' ) ); ?>"><?php esc_html_e( 'Edit', 'dream-university-management' ); ?></a> |
